...
void loop::add(socket& socket, Callable callable,
...
void loop::add(raw_socket_t const descriptor, Callable callable,
               short const event /* = POLL_IN */)
{
...
loop::timer_id_t loop::add(std::chrono::milliseconds delay, size_t times,
                           Callable callable)
{
...
void loop::remove(socket_t const& socket)
{
    ...
    items_.erase(std::remove_if(items_.begin(),
                                items_.end(), [&socket](const PollItemCallablePair & pair) -> bool
    {
...
void loop::remove(raw_socket_t const descriptor)
{
    ...
    items_.erase(std::remove_if(items_.begin(),
                                items_.end(), [descriptor](const PollItemCallablePair & pair) -> bool
    {
        const zmq_pollitem_t& item = pair.first;
...
ZMQPP_EXPORT int loop::poll(int event)
{
    int i = 0;
...
bool loop::start_handle_poller()
{
    for (const PollItemCallablePair& pair : items_)
    {
        ...
        if (poller_.has_input(pollitem) || poller_.has_error(pollitem) ||
                poller_.has_output(pollitem))
            if(!pair.second())
            {
                return false;
            }
...
long loop::tickless()
{
    ...
    long timeout = std::chrono::duration_cast<std::chrono::milliseconds>
                   (tick - std::chrono::steady_clock::now()).count();
...
bool loop::TimerItemCallablePairComp(const TimerItemCallablePair& lhs,
